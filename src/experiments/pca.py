...
for loss_fn_name in ['preg_loss', 'lap_loss', 'none']:
    for model_name in ['gcn']:
        ...
        pca_features = pca.fit(embeddings.detach().numpy()).transform(embeddings)
        for c in data.y.unique():
            plt.scatter(pca_features[data.y == c][:, 0], pca_features[data.y == c][:, 1])
        plt.savefig('reports/pca_' + loss_fn_name + '.png', dpi=300)
